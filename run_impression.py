...
def process_file(file_path, file_number):
    ...
    # print(f"parquet {file_path} read")
    # # add coloumns for the impression outputs
    # for col in ["cosy", "hsqc", "hmbc", "imp_atoms_df", "imp_pairs_df"]:
    #     if col not in data.columns:
    #         data[col] = None
    ...
    start_imp = time.time()
    impression_output = imp.predict_from_files(smiles)
    end_imp = time.time()
    
    print(f"\n\nIMPRESSION CALL TIMING\n\ntime spent on impression call: {end_imp - start_imp:.2f} seconds\n\n")
    print(f"The length of the parquet is {len(data)}")

    output_path = Path(f"../../alberts/multimodal-spectroscopic-dataset/data_imp/parquet_{file_number}")
    output_path.mkdir(parents=True, exist_ok=True)
    impression_output[0].to_csv(f"{output_path}/atoms.csv", index=False)  # index=False prevents writing row numbers
    impression_output[1].to_csv(f"{output_path}/pairs.csv", index=False)
...
def main():
    ...
    file_path = Path(f"../../alberts/multimodal-spectroscopic-dataset/data_new/aligned_chunk_{file_number}.parquet")
